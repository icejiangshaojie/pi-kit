...
function getResultOutput(result: SingleResult): string {
	if (isFailedResult(result)) {
		return truncateOutput(result.errorMessage || result.stderr || getFinalOutput(result.messages) || "(no output)");
	}
...
async function runSingleAgent(
	...
	// Specialized agents receive their own compact prompt and explicit tools.
	// Do not recursively load project instructions, skills, or the full extension set.
	const args: string[] = ["--mode", "json", "-p", "--no-session", "--no-context-files", "--no-skills", "--no-extensions"];
	...
	const emitUpdate = () => {
		if (onUpdate) {
			onUpdate({
				content: [{ type: "text", text: truncateOutput(getFinalOutput(currentResult.messages) || "(running...)") }],
